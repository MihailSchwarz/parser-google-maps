...
async function runGoogleMaps(searchQuery) {
  ...
  let attempts = 0;
  const maxAttempts = 3; // 1 основная + 2 дополнительные попытки
  ...
  while (attempts < maxAttempts) {
    try {
      ...
      try {
        ...
        if (!buttonFound) {
          ...
          if (attempts < maxAttempts) {
            console.log(
              `Попытка ${attempts} не удалась. Кнопка фото не найдена. Повторная попытка...`
            );
            if (context) await context.close();
            await new Promise((resolve) => setTimeout(resolve, 2000));
            continue; // Перезапускаем весь процесс
          }
      ...
      // Если дошли до этого места без ошибок, значит всё успешно
      break;
    } catch (error) {
